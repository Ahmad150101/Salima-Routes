...
test('keeps MapLibre LTR and Arabic marker overlays RTL', () => { const css = read('css/styles.css'); assert.match(css, /#map,.maplibregl-map\{direction:ltr\}/); assert.match(css, /customer-popup,.customer-map-marker,.customer-marker-label\{direction:rtl;text-align:right\}/); assert.match(css, /-webkit-line-clamp:2/); });
test('allows only the fixed RTL plugin host in CSP', () => { const html = read('index.html'); const config = read('js/config.js'); assert.match(config, /@mapbox\/mapbox-gl-rtl-text@0\.3\.0/); assert.match(html, /script-src[^;]*https:\/\/unpkg\.com/); assert.doesNotMatch(html, /script-src[^;]*\*/); });
